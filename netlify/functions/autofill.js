exports.handler = async (event) => {
  ...
  try {
    ...
    if (url.includes('rosario-fichas') || url.includes('jsonbin')) {
      ...
      if (!binId) return { statusCode: 400, body: JSON.stringify({ error: 'ID no encontrado' }) };
      ...
      let valor = null;
      if (d.precio) { const match = d.precio.replace(/\s/g, '').match(/[\d.]+/); if (match) valor = match[0]; }
      const desc = (d.descripcion || '').toLowerCase();
      const caract = (d.caracteristicas || []).join(' ').toLowerCase();
      const patio = (desc.includes('patio') || desc.includes('terraza') || desc.includes('jardin') || caract.includes('patio')) ? 'Sí' : 'No';
      const m2c = parseInt(f.supCubierta) || null;
      const m2t = parseInt(f.supTotal) || m2c;
      return { statusCode: 200, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ dir: d.direccion || d.titulo || null, dorm, m2c, m2t, patio, ubic: 'Frente', valor }) };
    }
